export class EventName
{
    public static readonly ShowScreen: string = 'ShowScreen';
    public static readonly BackScreen: string = 'BackScreen';
    public static readonly PlaySFX: string = 'PlaySFX';
    public static readonly PlayBGM: string = 'PlayBGM';
    public static readonly EndGame: string = 'EndGame';
    public static readonly Pause: string = 'Pause';
    public static readonly ReplayGame: string = 'ReplayGame';
    public static readonly AddIQ: string = 'AddIQ';
    public static readonly ChangeGameState: string = 'ChangeGameState';
}